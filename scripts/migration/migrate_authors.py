#!/usr/bin/env python3
"""
Migration script to convert from JSONB authors to relational authors table.
"""
...
async def run_migration():
    ...
    try:
        ...
        schema_file = Path(__file__).parent / "database" / "schema" / "06_authors.sql"
        migration_file = Path(__file__).parent / "database" / "schema" / "07_migrate_authors.sql"
        
        logger.info("Creating new author tables...")
        async with db_manager.get_connection() as conn:
            # Read and execute schema file
            with open(schema_file, 'r') as f:
                schema_sql = f.read()
            await conn.execute(schema_sql)
            logger.info("✓ Author tables created")
        ...
        logger.info("\n✅ Author migration completed successfully!")
